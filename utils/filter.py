...
def should_reserve(w, clean_level):
    if w == ' ':
        ...
        if clean_level == 'all':
            ...
            raise 'clean_level not support {}, please set for all, normal, clean'.format(clean_level)

def is_chinese(uchar):
    if u'\u4e00' <= uchar <= u'\u9fa5': return True
    else: return False

def is_number(uchar):
    if u'\u0030' <= uchar <= u'\u0039': return True
    else: return False

def is_alphabet(uchar):
    if (u'\u0041' <= uchar <= u'\u005a') or (u'\u0061' <= uchar <= u'\u007a'): return True
    else: return False
